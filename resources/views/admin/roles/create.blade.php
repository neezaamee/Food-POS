...
<div class="row g-4 mb-4">
    <div class="col-12 d-flex flex-wrap align-items-center justify-content-between gap-3">
        <div>
            ...
            <h4 class="mb-1 fw-bold text-body-emphasis">Create Custom Role</h4>
            <p class="text-muted mb-0 small">Define a specialized staff role and grant fine-grained permissions across system modules.</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('roles.index') }}" class="btn btn-outline-secondary">
                <i class="ph-duotone ph-arrow-left me-1"></i> Cancel
            </a>
        </div>
    </div>
</div>
...
<form method="POST" action="{{ route('roles.store') }}">
    ...
    <div class="card border-0 shadow-sm rounded-3 mb-4">
        <div class="card-header bg-transparent border-0 pt-3 px-4 pb-0">
            <h6 class="fw-bold mb-0"><i class="ph-duotone ph-identification-badge me-2 text-primary"></i> Role Information</h6>
        </div>
        <div class="card-body p-4">
            <div class="row g-3">
                <div class="col-md-6">
                    <label class="form-label small fw-semibold">Role Name <span class="text-danger">*</span></label>
                    <input type="text" name="name" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror" placeholder="e.g. Shift Supervisor, Floor Incharge" required autofocus>
                    @error('name')
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="description" value="{{ old('description') }}" class="form-control @error('description') is-invalid @enderror" placeholder="e.g. Supervises dining area and POS discounts">
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
        <div>
            ...
            <p class="text-muted small mb-0">Select specific privileges this role is authorized to execute.</p>
        </div>
        <div class="d-flex gap-2">
            ...
            <div class="card border-0 shadow-sm rounded-3 h-100">
                <div class="card-header bg-light bg-opacity-50 border-bottom py-2 px-3 d-flex justify-content-between align-items-center">
                    ...
                    <div class="form-check form-switch m-0">
                        <input class="form-check-input module-toggle" type="checkbox" role="switch" id="moduleToggle_{{ Str::slug($moduleName) }}" data-module="{{ Str::slug($moduleName) }}">
                    ...
                    <div class="row g-2">
                        ...
                        <div class="col-12 col-sm-6">
                            <label class="form-check d-flex align-items-start gap-2 p-2 border rounded-2 bg-body-tertiary bg-opacity-25 h-100 mb-0 cursor-pointer hover-bg-light" style="cursor: pointer;">
                                <input class="form-check-input flex-shrink-0 mt-1 perm-checkbox module-{{ Str::slug($moduleName) }}" type="checkbox" name="permissions[]" value="{{ $perm->id }}" id="perm_{{ $perm->id }}" {{ in_array($perm->id, old('permissions', [])) ? 'checked' : '' }}>
    ...
    <div class="d-flex justify-content-end gap-2 pb-5">
        ...
        <button type="submit" class="btn btn-primary px-4 d-flex align-items-center gap-2">
            <i class="ph-duotone ph-floppy-disk"></i> Create Role
        </button>
    </div>
</form>
